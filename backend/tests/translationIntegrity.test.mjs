...
assert.match(frontendSource, /const preFinalTranslation = Boolean\(/, "desktop translation results arriving before transcript_final remain eligible for React state updates");
assert.match(frontendSource, /preFinalTranslation \|\|/, "pre-final desktop results participate in current-event correlation");
assert.match(frontendSource, /!preFinalTranslation && !matchedPendingTranscript/, "pre-final desktop translation results bypass stale-original and stale-job guards");
assert.match(frontendSource, /new Set\(keepStreamingTranslation \? Object\.keys\(finalTranslationsRef\.current\) : \[\]\)/, "only same-utterance early translations bypass a late queued-state update");
assert.match(frontendSource, /incomingSequence > latestTranslationSequenceRef\.current/, "only a newer unmatched desktop translation may win the transcript race");
assert.match(frontendSource, /backend-\$\{sessionId \|\| "session"\}/, "pre-final translations receive a stable dubbing identity instead of reusing stale React state");
assert.match(frontendSource, /queueDubbingTranslations\(translationId, nextTranslations\)/, "the desktop race fix preserves automatic dubbing submission");
