...
// The free trial is exactly 180 seconds (3 minutes) of actual interpreter usage, not a
// calendar window: a brand-new normal user starts fully allotted and can use it immediately.
assert.equal(TRIAL_SECONDS, 180, "the trial is exactly 3 minutes of usage");
...
assert.equal(newUser.trialSecondsRemaining, 180, "a brand-new trial starts with the full 180 seconds");
...
assert.equal(subscriptionSnapshot(newUser, now).canUseInterpreter, true, "a fresh 180-second trial can use the interpreter");

// Using the interpreter for ~60 seconds leaves ~120 remaining, and this is computed purely
// from server-tracked state (previous remaining/used), never from anything client-supplied.
consumeTrialUsage(newUser, 60);
assert.equal(newUser.trialSecondsRemaining, 120, "60 seconds of usage leaves 120 seconds remaining");
assert.equal(newUser.trialSecondsUsed, 60, "60 seconds of usage is recorded as used");
assert.equal(subscriptionSnapshot(newUser, now).canUseInterpreter, true, "120 remaining seconds still allow interpreter access");
...
// Exhausting the 180 seconds blocks interpreter access and marks the account expired,
// exactly like the old date-based expiry did, just driven by usage instead of a clock.
consumeTrialUsage(newUser, 120);
...
assert.equal(overUsedUser.trialSecondsUsed, 180, "used seconds clamp at the 180-second total");
...
for (const role of ["admin", "super_admin"]) {
  ...
  const stillUnlimited = { role, status: "active", trialSecondsRemaining: 180 };
  consumeTrialUsage(stillUnlimited, 999);
  assert.equal(stillUnlimited.trialSecondsRemaining, 180, `${role} usage is never charged against a trial balance`);
}

// A paid subscriber is never restricted by the 180-second trial, and using the interpreter
...
// user who is mid-way through an unexpired old trial converts to a fresh 180-second usage
...
assert.equal(legacyActiveTrial.trialSecondsRemaining, 180, "a legacy trial that had not yet expired converts to a fresh 180-second allotment");
...
assert.equal(legacyNoTrialDates.trialSecondsRemaining, 180, "a legacy account with no prior trial dates gets a full 180-second trial");
...
consumeTrialUsage(adminManagedUser, 180);
assert.equal(adminManagedUser.status, "expired");
resetTrialUsage(adminManagedUser, now);
assert.equal(adminManagedUser.trialSecondsRemaining, 180, "reset_trial restores the full 180 seconds");
...
assert.equal(adminManagedUser.trialSecondsRemaining, 300, "add_trial_minutes adds on top of whatever remains");
